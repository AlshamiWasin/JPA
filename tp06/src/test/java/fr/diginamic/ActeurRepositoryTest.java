...
public class ActeurRepositoryTest {
	...
	public void testExtraireActeursParIdentite() {
		TypedQuery<Acteur> query = em.createQuery("SELECT DISTINCT a FROM Acteur a where a.identite = 'Marion Cotillard'", Acteur.class);
		List<Acteur> acteurs = query.getResultList();
	...
	public void testExtraireActeursParPays() {
		TypedQuery<Acteur> query = em.createQuery("SELECT DISTINCT a FROM Acteur a " +
				"JOIN FETCH  a.roles role " +
						"JOIN FETCH  role.film f " +
						"JOIN fetch  f.pays p where p.nom = 'France'", Acteur.class);
		List<Acteur> acteurs = query.getResultList();
	...
	public void testExtraireActeursParListePaysEtAnnee() {
		TypedQuery<Acteur> query = em.createQuery("SELECT DISTINCT a FROM Acteur a " +
				"JOIN FETCH  a.roles role " +
				"JOIN FETCH  role.film f " +
				"JOIN fetch  f.pays p where p.nom = 'France' and f.annee = 2017", Acteur.class);
		List<Acteur> acteurs = query.getResultList();
	...
	public void testExtraireParRealisateurEntreAnnee() {
		TypedQuery<Acteur> query = em.createQuery("SELECT DISTINCT a FROM Acteur a " +
				"JOIN FETCH  a.roles role " +
				"JOIN FETCH  role.film f " +
				"JOIN fetch  f.realisateurs r where r.identite = 'Ridley Scott' and role.film.annee between 2010 AND 2020", Acteur.class);
		List<Acteur> acteurs = query.getResultList();
